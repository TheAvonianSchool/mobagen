...
void World::print() {
  auto catposid =
      catPosition.y * (sideSize / 2) + catPosition.x + sideSize * sideSize / 2;
...
bool World::isValidPosition(const Point2D& p) {
  ...
  return (p.x >= -sideOver2) && (p.x <= sideOver2) && (p.y <= sideOver2) &&
         (p.y >= -sideOver2);
}
...
void World::OnDraw(SDL_Renderer* renderer) {
  ...
  t.position = {windowSize.x / 2 - (sideSize)*t.scale.x,
                windowSize.y / 2 - (sideSize - 1) * t.scale.y};
  auto catposid = (catPosition.y + sideSize / 2) * (sideSize) + catPosition.x +
                  sideSize / 2;
  for (int i = 0; i < worldState.size();) {
    if (catposid == i)
...
void World::OnGui(ImGuiContext* context) {
  ...
  if (ImGui::SliderInt("Side Size", &newSize, 3, 21) &&
      sideSize != (newSize / 2) * 2 + 1) {
    sideSize = (newSize / 2) * 2 + 1;
    clearWorld();
  }
  if (ImGui::SliderFloat("Turn Duration", &timeBetweenAITicks, 0.1, 30) &&
      sideSize != (newSize / 2) * 2 + 1) {
    sideSize = (newSize / 2) * 2 + 1;
    clearWorld();
  }
...
int World::getWorldSideSize() const { return sideSize; }
