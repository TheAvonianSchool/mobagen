...
class World : GameObject {
 private:
  ...
  // the world have 11x11 size, so it needs 121 bits to represent it. in other
  // words we need 16 bytes to fully represent it. bit representation: 0 empty,
  // 1 blocked. to represent the cat position we need only one byte. 4 bits for
  // X and another 4 for Y create a structure holding one byte for cat position
  // and 16 bytes for the blocked map.
  ...
  bool catCanMoveToPosition(Point2D pos);

  // check if catcher can move to the position required
  bool catcherCanMoveToPosition(Point2D pos);
